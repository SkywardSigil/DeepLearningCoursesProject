...
def load_humaneval(data_dir):
    file_path = os.path.join(data_dir, "HumanEval.jsonl")
    examples = []
...
def load_apps(data_dir):
    ...
    return DatasetDict({
        "train": data["train"],
        "validation": data["test"]  # APPS没有单独的validation集，使用test集作为验证
    })
...
def main():
    ...
    parser.add_argument(
        "--model_name", 
        type=str, 
        default=r"E:/NJU/深度学习平台/final/exp_new/models/codebert-base", 
        help="预训练模型名称或本地路径"
    )
